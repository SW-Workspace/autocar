...
export async function supabaseCreateRentals(
  rentalData: Partial<SB_RentalsModel>,
) {
...
export async function supabaseGetRentalsByStatus(
...
export async function supabaseUpdateRentalById(
  rentalId: number,
  updates: Partial<SB_RentalsModel>,
) {
  const { data, error } = await supabase
    .from("rentals")
    .update(updates)
