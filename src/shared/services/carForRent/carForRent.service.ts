...
export async function supabaseCreateCarForRent(
  carData: Partial<SB_CarForRentModel>,
) {
...
export async function supabaseGetAllCarsForRent() {
  const { data, error } = await supabase.from("car_for_rent").select("*");
...
export async function supabaseGetCarByRenderId(userRenderId: number) {
  const { data, error } = await supabase
    .from("car_for_rent")
    .select("*")
...
export async function supabaseGetCarByOwnerId(userOwnerId: number) {
  const { data, error } = await supabase
    .from("car_for_rent")
    .select("*")
...
export async function supabaseUpdateCarForRentById(
  carId: number,
  updates: Partial<SB_CarForRentModel>,
) {
  const { data, error } = await supabase
    .from("car_for_rent")
    .update(updates)
...
export async function supabaseRentCar(
  carId: number,
  userId: number,
  rentalDays: number,
) {
